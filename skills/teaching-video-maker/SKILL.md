...
默认按朋友圈 / 视频号发布使用，内容必须短，能直接复制发布。包含：

- `标题`：16 字以内，适合首行。
- `发布文案`：60 到 120 字，最多 2 个短段落。
- `话题标签`：0 到 3 个，按用户习惯可省略。
- `备用短句`：可选，40 字以内；只有确实有用时才写。

写作要求：

- 只写用户可发布的文案，不要写“本视频展示了”“界面包含”等制作视角表述。
- 不要复述整段旁白；把内容压缩成判断、收益和下一步行动。
- 默认不要写长要点清单、置顶评论、长摘要；除非用户明确要长版。
- 不要夸大视频里没有证明的结论。
...
- `output/<topic-slug>-summary.md` 存在，且内容短到可直接复制到朋友圈 / 视频号发布。
