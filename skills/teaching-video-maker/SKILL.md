...
6. **Generate voice**：用本地 VibeVoice 为每个 scene 生成音频，保存到 `audio/scene-XX.wav` 或 `audio/scene-XX.mp3`。
7. **Measure audio**：用 `ffprobe` 读取每段真实时长，生成 `timing.json`。不要只依赖估算时长。
8. **Build Remotion video**：调用 `remotion` skill 创建或更新 `remotion/` 项目，按 `timing.json` 编排画面和 `<Audio>`。
9. **Preview and fix sync**：用 Remotion preview 或 render 检查音画对应。旁白提到的概念，应在 0.5 秒内出现在画面上。
10. **Render final video**：渲染有声 mp4 到 `output/`。
