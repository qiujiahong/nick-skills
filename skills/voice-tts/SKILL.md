...
- `VOICE_TTS_API_KEY` — 火山引擎语音技术 API Key（必填）
- `VOICE_TTS_VOICE_TYPE` — 默认音色 ID（可选，默认 `BV001_V2`）
- `VOICE_TTS_CLUSTER` — 集群名称（可选，默认 `volcano_icl`）
...
```bash
# 使用复刻音色
python3 scripts/tts.py "从前有座山，山上有座庙" --voice S_hm8o8CxW1

# 使用预置音色，加快语速
python3 scripts/tts.py "今天天气真不错" --voice BV001_V2 --speed 1.2

# 输出 wav 格式
python3 scripts/tts.py "测试音频" --voice S_hm8o8CxW1 --encoding wav --output test.wav
```
